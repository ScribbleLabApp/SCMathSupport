...
///
/// The EigenvalueDecomposition class computes the eigenvalues and eigenvectors of a real symmetric matrix `A`. It decomposes the matrix into the form `A = V * D * V^T`, where `D` is a diagonal matrix containing the eigenvalues and `V is a matrix whose columns are the corresponding eigenvectors. This decomposition is fundamental in various numerical methods, including solving linear systems, principal component analysis (PCA), and stability analysis of dynamical systems.
///
///    ```swift
...
///
/// Both algorithms work iteratively to converge on accurate eigenvalues and eigenvectors. The resulting eigenvalues are stored in `eigenvalues`, and the corresponding eigenvectors are stored in `eigenvectors`.
///
...
public class EigenvalueDecomposition {
    ...
    ///
    private func tql2(a: inout [[Double]], n: Int, d: inout [Double], e: inout [Double], z: inout [[Double]]) {
        ...
        for l in 0..<n {
            ...
            if m > l {
                ...
                repeat {
                    ...
                    var g = d[l] // FIXME: Change var to let again
